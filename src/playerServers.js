...
export async function main(ns) {
  ...
  ns.tprint(`Targeting a max of ${settings.maxGbRam} GiB.`);
  ...
  while (true) {
    ...
    // ns.tprint(`[${localeHHMMSS()}] Have ${purchasedServers.length} servers, chosen action is ${action} .`)
    ...
    if (action == settings.actions.BUY) {
      ...
      targetRam = Math.min(targetRam, settings.maxGbRam)
      // ns.tprint(`[${localeHHMMSS()}] Smallest server ${smallestCurrentServer} biggest server ${biggestCurrentServer} target ram ${targetRam} .`)
